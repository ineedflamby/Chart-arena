...
/**
 * v5.1: Handle deposit request — operator pulls approved MOTO on behalf of player.
 * Player must have called MOTO.increaseAllowance(escrow, amount) first on the frontend.
 * The backend calls operatorPullDeposit with the operator wallet (real keys = simulation works).
 */
async function handleDepositRequest(ws: GameSocket, msg: Record<string, unknown>, wsId: string): Promise<void> {
    ...
    sendToSocket(ws, ServerMsg.DEPOSIT_STATUS, { status: 'pulling', amount: amountStr });
    logger.info(TAG, `Deposit request: ${address} wants to deposit ${amount} (${Number(amount) / 1e18} MOTO)`);
    try {
        const txHash = await contractService.operatorPullDeposit(address, amount);
        logger.info(TAG, `Deposit TX broadcast for ${address}: ${txHash}`);
        sendToSocket(ws, ServerMsg.DEPOSIT_STATUS, { status: 'broadcast', amount: amountStr, txHash });
        // Don't wait for confirmation — the balance will update when the block mines
    } catch (err) {
